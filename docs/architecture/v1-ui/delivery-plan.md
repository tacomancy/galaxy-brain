...
registered decisions, the full silent packaged suite passes 26/26 specs, and
the repository check, coverage, complexity, documentation, and changed-lines
gates pass. Changed-lines coverage is 85.96% (49/57 applicable lines) when
run with `--repository-root .. --coverage-root .` from `app/`. The remaining
release work is the broader provider-free evidence checklist, explicit
privacy/non-retention review, and human acceptance of the real packaged
application.
