...
- Persist only the last explicitly selected repository root for resume convenience; validate it before use, and never scan sibling directories or silently substitute another root.
- Keep initial Agent Provider configuration in the machine-local application `.env` file described by [`app/.env.example`](../../app/.env.example), outside the selected repository's portable format, audit records, logs, proposals, and source content. Never log, echo, commit, or copy its values; the repository-wide ignore rules exclude `.env`. Treat this as a temporary V1 mechanism pending OS-backed storage or provider-native OAuth.
...
- Use the shared `agent-provider-unavailable` outcome for every operation that requires an Agent Provider. Preserve provider-independent work and captured material; never replace an unavailable agentic result with a blank, partial, or apparently successful result.
- Require explicit per-operation confirmation before every OpenAI request, including user-only prompts. Prepare and display a concise summary and inspectable exact payload before calling the Model Adapter; allow removal of whole context items with regenerated previews, but not arbitrary inline redaction; cancellation must make no request and preserve local state, and the Adapter must not add context after approval. Do not add blanket consent, remembered consent, silent background requests, or whole-repository uploads in V1. Configurable confirmation policies are deferred.
- Treat OpenAI request and response bodies as transient sensitive data. Do not persist them in automatic history, caches, logs, audit records, or support files; persist an agent result only through an explicit user save to an ordinary repository artifact. Tests must prove non-retention and preserve diagnostic metadata without reconstructing content.
- Explicitly saved OpenAI results must carry agent-generated attribution, provider identity, pinned model, generation timestamp, operation, and applicable Source Record or Source Locator references. The default save excludes the prompt/context; a separate explicit save-with-prompt/context operation may retain the human-facing prompt, selected source references or locators, and concise context summaries as a point-in-time snapshot, but not full source excerpts or the hidden full API payload. A mismatch in saved versus current source identity or content identity may affect navigation targets but must not silently rewrite the saved snapshot; opening it must show a non-blocking stale-context warning. Preserve this provenance through human edits and keep the artifact in Working Material until Governance applies a Proposal.
...
- If the current source for a saved context snapshot cannot be checked or lacks a comparable identity, report `source status unavailable`, preserve the snapshot, and do not claim that it is current.
- An explicit refresh of a saved context snapshot must create a new snapshot/version and preserve the original; it must not silently replace the historical context in place.
- A snapshot refresh updates only saved context representation. Result regeneration is a separate operation, and any new OpenAI request requires fresh confirmation of its final payload.
- Result regeneration must create a new result version and preserve the previous result; it must not silently overwrite earlier agent output.
- Present the newest result as current and retain prior versions through ordinary artifact history rather than separate top-level items for each regeneration.
- An explicit restore of an older result must create a new current version derived from it, preserve all intervening versions, and make no OpenAI request.
- Retain prior agent-result versions by default; prohibit automatic cleanup. Any future deletion or history pruning must require explicit approval and explain the lost recovery and provenance.
