...
Use it for architecture, ADRs, engineering and agent guidance, application design proposals under `app/docs/proposals/`, review records under `app/docs/reviews/`, and other durable project documentation. Knowledge-base content belongs under the selected repository's `assets/`, `knowledge/`, `projects/`, `proposals/`, `scratch/`, `sources/`, or `templates/` roots according to its role and lifecycle. The public project contains only the synthetic fixture under `app/tests/fixtures/knowledge-repository/` and starter skeleton under `app/templates/knowledge-repository/`.
