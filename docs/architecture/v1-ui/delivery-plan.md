...
- **Public Behavior:** The packaged Workbench presents explicit Search, Ask, and Jump modes. Search reads the selected repository locally and returns authority-bearing matches; Ask prepares an exact, cited payload, regenerates it after whole-item context removal, requires explicit confirmation before the narrow Model Adapter boundary, and reports answered, unsupported, declined, canceled, invalid, and provider-unavailable outcomes; Jump resolves known workspace/topic/Source Record targets without provider or repository mutation.
...
- **Implementation:** Added the Discovery Module, file-backed discovery Adapter, deterministic fixture Model Adapter, typed main/preload operations, shared renderer Discovery surface, authority-bearing results, exact Ask preview and confirmation boundary, known Jump transitions, focused S4/S5 coverage, and the packaged S1 workflow.
