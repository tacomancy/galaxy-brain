...
| DES-005 | Adding a PDF did not let the user choose between repository-managed storage and an external OS file. | proposed | Add an “Add PDF” flow, discoverable as “Import PDF,” with `managed-lfs` and `linked-local` Source Asset modes. Neither mode commits a machine-specific path. See the [Repository Format proposal](../proposals/2026-08-26-git-backed-knowledge-repository.md). |
| DES-006 | A remote Git repository could be mistaken for a complete backup despite separate Git refs and LFS-object availability. | proposed | The design now refuses to claim that a remote is a verified backup; complete backup automation remains deferred. See the [Repository Format proposal](../proposals/2026-08-26-git-backed-knowledge-repository.md#deferred-work) and [Git/LFS research](../sources/web/2026-08-26-git-lfs-backend-research.md#open-questions). |

## Unresolved implementation and policy findings

| ID | Finding | Status | Disposition and controlling record |
| --- | --- | --- | --- |
| OPEN-001 | Which asset paths or file types should be eligible for Git LFS, and whether each tracking-rule change requires explicit approval. | decision-needed | Evidence and the open question are recorded in [Git/LFS research](../sources/web/2026-08-26-git-lfs-backend-research.md#open-questions). The [Repository Format proposal](../proposals/2026-08-26-git-backed-knowledge-repository.md#deferred-work) defers concrete patterns, thresholds, and migration policy. |
| OPEN-002 | Whether the application bundles maintained Git and Git LFS clients or validates system installations. | decision-needed | The choice remains subject to packaging and security support. See [Git/LFS research](../sources/web/2026-08-26-git-lfs-backend-research.md#open-questions) and the [Repository Format proposal](../proposals/2026-08-26-git-backed-knowledge-repository.md#deferred-work). |
| OPEN-003 | Whether GitHub authentication relies entirely on Git credential helpers or requires GitHub-specific application integration. | decision-needed | V1 provider integration is outside the proposed repository interface. The unresolved choice is recorded in [Git/LFS research](../sources/web/2026-08-26-git-lfs-backend-research.md#open-questions). |
| OPEN-004 | Which backup target and verification cadence guarantee recovery of required Git refs and reachable LFS objects independently of GitHub. | decision-needed | Backup automation is deferred; the operational question remains in [Git/LFS research](../sources/web/2026-08-26-git-lfs-backend-research.md#open-questions). |
| OPEN-005 | The concrete machine-local configuration format and UI for resolving linked PDFs and changing storage modes are unspecified. | deferred | They remain implementation details constrained by portable Source Records, no committed absolute paths, explicit mode changes, and preservation on failure. See the [Repository Format proposal](../proposals/2026-08-26-git-backed-knowledge-repository.md#deferred-work). |
...
- Approve, reject, revise, or apply the two pending proposals through their own exact approval gates.
