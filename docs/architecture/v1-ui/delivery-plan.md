...
The implementation entry point is the [TB12 specification](tracer-bullet-12-spec.md). Its explicit preparation task was completed before implementation: governing documentation and ADRs were reviewed; the S4/S1 behavior, literal fixture values, seams, minimum vertical path, deferrals, and discarded alternatives were recorded; and the specification was published to Issue #31. The implementation uses a deterministic fixture Model Adapter and leaves live production OpenAI transmission explicitly deferred.

#### TB12 implementation record — September 1, 2026

- **Public Behavior:** The packaged Workbench presents explicit Search, Ask, and Jump modes. Search reads all supported portable Source Record categories and saved JSON results locally and returns authority-bearing matches; Ask presents an explicit context-selection list, prepares an exact cited payload, regenerates it after whole-item context removal, requires explicit confirmation before the narrow Model Adapter boundary, and reports answered, unsupported, declined, canceled, invalid, repository-unavailable, and provider-unavailable outcomes; Jump resolves known workspace and discoverable-item targets without provider or repository mutation.
- **Test Seam:** S4 Discovery Module tests use the real Module with an independently defined deterministic repository and narrow fixture Model Adapter; supporting file-backed Adapter coverage reads the checked-in fixture; S1 uses the silent packaged Electron/WebdriverIO workflow.
- **Red/Green evidence:** The first packaged run found a front-matter allow-list parser defect; the corrected Adapter now reads the fixture's topic, Source Record, and two annotations. A second workflow pass required opening the exact-payload disclosure before asserting its contents. The focused S4 suite then passed 5 tests, the full Vitest suite passed 133 tests, the complete `npm run check` gate passed, coverage passed at 83.52% statements / 75.18% branches / 97.10% functions / 83.40% lines, and the complete packaged workflow passed all 29 WebdriverIO specs.
- **Implementation:** Added the Discovery Module and separate in-memory/file-backed discovery Adapters, deterministic fixture Model Adapter, typed main/preload operations, shared renderer Discovery surface with explicit Ask context controls and accessible mode panels, authority-bearing results, exact Ask preview and confirmation boundary, stable-ID Jump transitions, focused S4/S5 coverage, and the packaged S1 workflow.
- **Scope confirmed:** Semantic/vector indexing, fuzzy ranking, live production OpenAI transmission, provider configuration UI, model selection, arbitrary Jump commands, multi-repository indexes, Ask-specific save UI, and broader Discovery formats remain explicitly deferred. Existing Synthesis persistence rules remain the authority for any later explicit saved agent result.
- **Acceptance:** Automated implementation evidence is complete. The user passed all eight packaged human-acceptance gates and explicitly accepted TB12 on September 1, 2026.
...
The implementation entry point is the [TB14 specification](tracer-bullet-14-spec.md). It records TB14 as dependent on the merged TB13 Atlas surface, defines the exact fixture suggestion and evidence, separates suggestion from confirmation/correction, and preserves the explicit unavailable-provider boundary.

#### TB14 implementation record — September 1, 2026

- **Public Behavior:** Atlas presents an evidence-backed learning-progress suggestion with the current stage “Evidence updates”, proposed stage “Posterior belief”, both saved annotation evidence items, the fixture Source Record, and an explicit pending-human-action status. Passive rendering and navigation do not advance the stage; explicit confirmation advances the session projection, while explicit correction preserves the prior stage and records the correction.
- **Test Seam:** S1 packaged Electron workflow through the real main process, typed preload bridge, Learning Module, deterministic fixture State and Suggestion Adapters, unavailable-provider Adapter, Workbench Session, Knowledge Repository, and Atlas/Studio UI Adapters; focused S2 Module tests cover evidence validation, pending state, confirmation, correction, repeated-operation boundaries, and unavailable-provider outcomes.
- **Red evidence:** The focused TB14 contract and packaged workflows were written against the missing Learning Module/Atlas behavior; the first focused run failed on the absent learning surface and operations before implementation supplied the vertical path.
- **Green evidence:** With Node.js `24.19.0`, the focused Learning Module suite passes 8 tests covering pending state, confirmation, correction, repeated-operation boundaries, evidence validation, absent state, passive reads, and unavailable-provider outcomes. The complete packaged Electron workflow passes 31/31 specs, including the TB14 confirmation/correction workflow; the dedicated provider-unavailable TB14 workflow passes 1/1 spec.
- **Implementation:** Added the framework-independent Learning Module, deterministic fixture and unavailable-provider Adapters, typed main/preload operations, configurable S1 test mode, Atlas progress suggestion/confirmation/correction presentation, focused S1 workflows, and the code-map entry.
- **Commands:** `npm run check`, `npm run test:coverage`, `npm run lint:complexity`, `npm run test:workflow`, and `GALAXY_BRAIN_WDIO_TEST_MODE=provider-unavailable npx wdio run ./wdio.conf.ts --spec ./tests/workflows/tb14-learning-progress.e2e.ts` all pass with Node.js `24.19.0`.
