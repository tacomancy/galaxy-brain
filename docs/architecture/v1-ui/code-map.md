...
| [Source Processing](architecture.md#source-processing-module) | Add PDFs with a chosen Source Asset mode, capture located annotations, report availability, relink, and request Synthesis | S3 | [`app/src/modules/source-processing/index.ts`](../../../app/src/modules/source-processing/index.ts) | TB5 located source-claim capture plus TB7 preview, context removal, confirmed handoff, provider-unavailable outcome, transient result handling, default save, opt-in prompt/context snapshot, non-blocking stale-context warning, and unavailable-source-status outcome implemented; S1 confirmation UI, real provider integration, file-backed result persistence, source refresh/versioning, import, availability, and relinking remain deferred |
...
| --- | --- | --- | --- |
| Workbench shell | Window-level layout, global mode controls, accessible navigation, and workspace composition | [`app/src/renderer/`](../../../app/src/renderer/) | Fresh-session shell, TB2/TB3 repository selection and resume state, TB4 workspace composition, TB6 meaningful Paper Desk resume, and TB6.1–TB6.3 promoted workspace presentations implemented; richer workspace behavior remains deferred |
| Atlas | Orientation, continuation, Judgment queue, Learning Routes, and actionable derived state | [`app/src/renderer/atlas/Atlas.tsx`](../../../app/src/renderer/atlas/Atlas.tsx) | Fresh empty state, repository selection/access status, failure feedback, resume state, TB4 topic continuation, and TB6.1 promoted continuation surface implemented; later Atlas behavior remains unimplemented |
| Studio | Knowledge Authoring Interface and inspector presentation | [`app/src/renderer/studio/Studio.tsx`](../../../app/src/renderer/studio/Studio.tsx) | TB4 contextual topic presentation, Source Record transition, and TB6.2 promoted topic/Working Material surface implemented; authoring behavior remains unimplemented |
...
| Workbench bridge | Narrow renderer-to-main desktop operations exposed through `contextBridge` | [`app/src/preload/workbench-bridge.ts`](../../../app/src/preload/workbench-bridge.ts) | Fresh session, repository create/open, TB4 context-transition, and TB6 saved-annotation operations implemented |

## External Adapters

| Seam | Production Adapter | Test Adapter | Test Seam | Status |
| --- | --- | --- | --- | --- |
| Knowledge Repository | [`app/src/adapters/knowledge-repository/file-backed-knowledge-repository.ts`](../../../app/src/adapters/knowledge-repository/file-backed-knowledge-repository.ts) stages, validates, opens the bundled repository format, and reads the TB4 fixture context and TB5 annotation | [`app/src/adapters/knowledge-repository/in-memory-knowledge-repository.ts`](../../../app/src/adapters/knowledge-repository/in-memory-knowledge-repository.ts) | S5 contract; used by S1–S4 | File-backed creation, opening, safety validation, compatibility outcomes, context and saved-annotation discovery, and contract coverage implemented through TB6 |
| Machine-local session state | [`app/src/adapters/session-state/file-backed-workbench-session-state.ts`](../../../app/src/adapters/session-state/file-backed-workbench-session-state.ts) stores exact-root resume, active workspace, and Source Record reading position outside the portable format | Isolated session-state path supplied by the S1 workflow harness | S1/S5 | TB3 exact-root persistence and recovery plus TB6 active Paper Desk and reading-position persistence implemented; broader restorable Working Set state remains deferred |
| PDF | Deferred engine under `app/src/adapters/pdf/` | [`app/src/adapters/pdf/fixture-pdf-adapter.ts`](../../../app/src/adapters/pdf/fixture-pdf-adapter.ts) | S5 contract; used by S3 | TB5 fixture passage resolution implemented; production PDF engine remains unselected |
| Working Material | [`app/src/adapters/working-material/file-backed-working-material-repository.ts`](../../../app/src/adapters/working-material/file-backed-working-material-repository.ts) | [`app/src/adapters/working-material/in-memory-working-material-repository.ts`](../../../app/src/adapters/working-material/in-memory-working-material-repository.ts) | S3/S5 | TB5 source-claim annotation persistence plus TB7 default saved-result, opt-in prompt/context snapshot, and stale-context preservation contracts through the S3 result Adapter implemented; file-backed synthesis-result persistence remains deferred |
