...
Status: Tracer Bullets 1 through 7 and TB6.1–TB6.3 are complete and accepted. TB8 S2/S5 implementation and compatible packaged verification are complete, with human acceptance pending. The first TB9 stale-Judgment S2 slice is implemented and tested on a feature branch, with human acceptance pending; it is not part of current `main` or release `0.9.0`. Later tracer bullets remain unimplemented.
...
| [`app/tests/governance/`](../../../app/tests/governance/) | S2 Governance behavior tests | [`app/tests/governance/apply-governed-change.test.ts`](../../../app/tests/governance/apply-governed-change.test.ts) | TB8 S2/S5 behavior is implemented and tested on `main`; the first TB9 stale-Judgment behavior test at `app/tests/governance/reject-stale-judgment.test.ts` is implemented and tested on a feature branch, not yet merged |
...
| [Governance](architecture.md#governance-module) | Draft Proposals, record Judgment, and apply eligible exact-version changes through file transactions | S2 | [`app/src/modules/governance/index.ts`](../../../app/src/modules/governance/index.ts) | TB8 S2 Proposal/Judgment/application and S5 file-backed persistence are implemented and tested; compatible packaged verification is complete and human acceptance is pending. The first TB9 stale-Judgment S2 slice is implemented and tested on a feature branch, not yet merged to current `main`; human acceptance is pending |
...
| --- | --- | --- | --- |
| Workbench shell | Window-level layout, global mode controls, accessible navigation, and workspace composition | [`app/src/renderer/`](../../../app/src/renderer/) | Fresh-session shell, TB2/TB3 repository selection and resume state, Issue 51 context selection, TB4 workspace composition, TB6 meaningful Paper Desk resume, and TB6.1–TB6.3 promoted workspace presentations implemented; richer workspace behavior remains deferred |
| Atlas | Orientation, continuation, Judgment queue, Learning Routes, and actionable derived state | [`app/src/renderer/atlas/Atlas.tsx`](../../../app/src/renderer/atlas/Atlas.tsx) | Fresh empty state, repository selection/access status, failure feedback, resume state, Issue 51 explicit context choices, TB4 topic continuation, and TB6.1 promoted continuation surface implemented; later Atlas behavior remains unimplemented |
...
| Workbench bridge | Narrow renderer-to-main desktop operations exposed through `contextBridge` | [`app/src/preload/workbench-bridge.ts`](../../../app/src/preload/workbench-bridge.ts) | Fresh session, repository create/open, Issue 51 explicit context-selection, TB4 context-transition, TB6 saved-annotation operations, and TB7 exact-preview, context-removal, confirmation, saved-result read, and restore operations implemented |
